...
class WikiRandom {
    ...
    public function getId() {
        if (empty($this->article_ids)) throw new Exception('Empty article_ids variable');
    ...
    public function getArticleImages($i = null) {
        $result = array();
        ...
        foreach ($article_ids as $page_id) {
            ...
            $wiki_api = $this->base_api.'action=query&generator=images&prop=imageinfo&iiprop=url&pageids='.$this->getId();
            $json_wapi = json_decode(file_get_contents($wiki_api), true);
            foreach ($json_wapi['query']['pages'] as $item) {
                $result[$page_id][] = $item['imageinfo'][0]['url'];
            }
        }
    ...
    public function getBulkData($limit = 1, $sentences = 5, $chars = 200, $with_images = false, $image_min_size = 102400) {
        $wiki_api = $this->base_api.'action=query&generator=random&grnnamespace=0&grnlimit='.$limit.'&prop=info|extracts&inprop=url';
        if ($sentences > 0) {
        ...
        if ($with_images) {
            foreach ($this->article_ids as $page_id) {
                ...
                $json_wapi = json_decode(file_get_contents($wiki_api), true);
                if (!isset($json_wapi['query'])) var_dump($json_wapi);
                $images = array();
                foreach ($json_wapi['query']['pages'] as $item) {
                    $imageinfo = $item['imageinfo'][0];
                    if ($image_min_size > 0 && $imageinfo['size'] < $image_min_size) continue;
